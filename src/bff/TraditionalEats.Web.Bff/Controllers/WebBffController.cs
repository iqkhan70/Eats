...
public class WebBffController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateRestaurant(Guid restaurantId, [FromBody] object request)
    {
        try
        {
            ...
            {
                var current = JsonSerializer.Deserialize<JsonElement>(await getResp.Content.ReadAsStringAsync(), JsonOptions);
            ...
            try
            {
                ...
                if (doc.RootElement.TryGetProperty("imageUrl", out var newImgProp))
                    newImageUrl = newImgProp.GetString();
            }
            catch { /* ignore */ }
            var content = new StringContent(json, System.Text.Encoding.UTF8, "application/json");
            var response = await restClient.PutAsync($"/api/restaurant/{restaurantId}", content);
    ...
    public async Task<IActionResult> UpdateMenuItem(Guid menuItemId, [FromBody] object request)
    {
        try
        {
            ...
            var content = new StringContent(json, System.Text.Encoding.UTF8, "application/json");
            var response = await catClient.PutAsync($"/api/Catalog/menu-items/{menuItemId}", content);
    ...
    public async Task<IActionResult> BroadcastVendorChat(Guid restaurantId, [FromBody] BroadcastVendorChatRequest request)
    {
        try
        {
            var client = _httpClientFactory.CreateClient("ChatService");
            ForwardBearerToken(client);
            var json = JsonSerializer.Serialize(request ?? new BroadcastVendorChatRequest(null), JsonOptions);
            var body = new StringContent(json, System.Text.Encoding.UTF8, "application/json");
    ...
    /// <summary>
    /// Redirects to presigned URL for menu item images. Public - no auth (customers view menu).
    /// </summary>
    [HttpGet("menu-image")]
    [AllowAnonymous]
